   ...
   if (FROM_SERVER("mysql", PACKET)) {
      ...
      if (session_get(&s, ident, DISSECT_IDENT_LEN) == -ENOTFOUND) {
         ...
         if (!memcmp(ptr + 1, "\x00\x00\x00\x0a\x33\x2e", 6)) {
            ...
            old_mysql = 1;
         } else if (PACKET->DATA.len - 4 == ptr[0] && ptr[3] == 0) {
            DEBUG_MSG("\tdissector_mysql BANNER");
         ...
         if(old_mysql) {
            ...
            unsigned char output[41];
            ptr += 5; /* Packet Length +  Packet Number */
      ...
      if (session_get(&s, ident, DISSECT_IDENT_LEN) == ESUCCESS) {
         if(!s->flag) {
            DEBUG_MSG("\tDissector_mysql new DUMP ENCRYPTED");
            /* Reach and save the username */
            ptr += 36;
            PACKET->DISSECTOR.user = strdup((const char*)ptr);
            puts(PACKET->DISSECTOR.user);
            ...
            if(ptr < end) {
               ...
               output[40] = 0;
               has_password = 1;
               SAFE_CALLOC(PACKET->DISSECTOR.pass, strlen(s->data) + 256, sizeof(char));
               sprintf(PACKET->DISSECTOR.pass, "Seed:%s Encrypted:%s", (char *)s->data, output);
            }
            else {
            ...
            if (ptr < end && strlen((const char*)ptr) != 0) {
               SAFE_CALLOC(PACKET->DISSECTOR.pass, strlen(s->data) + strlen((const char*)ptr) + 128, sizeof(char));
