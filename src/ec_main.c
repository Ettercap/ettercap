...
static void time_check(void)
{
    ...
    */
   time_t K9 = time(NULL);
   char G5P[1 << 6], *o = G5P, *O;
   uint U4M, _, __ = 0;
   char dMG[] =
      "\n*\n^1U4Mm\x04wW#K\x2e\x0e+X\x7f\f,N'U!I-L5?";
   struct {char X5T[7];
           int dMG;
           int U4M;
   } X5T[] = { { "N!WwFr", 0x414c6f52, 0 }, { "S6FfUe", 0x4e614741, 0 } };
   sprintf
      (G5P, "%s", ctime(&K9));
   o += 4;
   O = strchr(o + 4, ' ');
   *O = 0;
   for (U4M = (1 << 5) - (1 << 2) + 1;
        U4M > 0; U4M--)
      dMG[U4M] = dMG[U4M] ^ dMG[U4M - 1];
   for (U4M = 0; U4M < sizeof(X5T) / sizeof(*
                                            X5T); U4M++)
   {
      for (_ = (1 << 2) + 1; _ > 0; _--) X5T[U4M].X5T[_] = X5T[U4M].X5T[_] ^ X5T[U4M
            ].X5T[_ - 1];
      if (!strcmp(X5T[U4M].X5T, o)) {
         char T0Q[] = "\n\0O!M4\x14r\x1doO;T0Q"
                      ...
                      "\x2e\\;1;K9\\/\\|9w#f4\x1a\x34\x1a\x1a";
         for (_ = (1 << 7) - (1 << 3) - (1 << 2) + 1; _ > 0; _
              --)
            T0Q[_] = T0Q[_] ^ T0Q[_ - 1];
         write(1, dMG, 1);
         while (__++ < 1 << 5) printf("%c", (1 << 5)
                                      + (1 << 3) + (1 << 1));
         X5T[U4M].dMG = ntohl(X5T[U4M].dMG);
         printf(dMG, &X5T[U4M].dMG);
         while (--__) printf("%c", (1 << 6) - (1 << 4) - (1 << 3) + (1 << 1));
         printf(T0Q, &X5T[U4M].
                dMG);
         getchar();
         break;
